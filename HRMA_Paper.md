...
where $\lambda$ is a nozzle divergence correction factor: $\lambda = 0.985$ for bell (Rao-optimized) nozzles, $\lambda = 0.975$ for parabolic contours, and $\lambda = 0.983$ for conical nozzles with a 15-degree half-angle. The conical value follows directly from the standard relation $\lambda = (1 + \cos\alpha)/2$ with $\alpha = 15^{\circ}$, giving $\lambda = (1 + 0.9659)/2 = 0.98296 \approx 0.983$. The specific impulse follows directly as $I_{sp} = C_F C^* / g_0$, where $g_0 = 9.80665$ m/s$^2$ (BIPM standard).
