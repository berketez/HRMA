...
|---|---|---|
| Paraşüt üçlüsü (parachute_area/cd/deploy_delay) hibritte KAPISIZ: API bağlı (app.py:1778-1780) ama hibrit formunda alan yok — katıda var (solid.html:1824-1831); 2,0 m²/1,4/2,0 s varsayılanını kullanıcı düzeltemiyor (assumed=True beyanlı ama düzeltilemez; parti 22 borç ayıklamasında ölçüldü) | "Kanal var kapı yok" sınıfı — hibrit formuna alan üçlüsü bağlanacak (katıdaki desenle); parti 25 adayı | bu defter; ayrıntı parti 22 ajan raporu + `kalan_supheliler.txt` |
...
| Katı çözücüsü `port_area` dizisini içeride tutuyor ama `_published_thrust_curve` (solid_rocket_engine.py:8664) dışarı vermiyor — animasyon kütle-denge türetmesiyle çalışıyor; `'port_area': _al('port_area')` yayımlanırsa türetme gereksizleşir ve BATES dışı taneler de gerçek eşdeğer port yarıçapıyla animasyonlanır | Motor sonuç sözleşmesine alan ekleyen değişiklik — kendi partisinde, sarsım+animasyon bekçileriyle | bu defter; parti 25 B5 ajan raporu |
| 3B ızgara rozeti (`_gridBadge`) hâlâ ekran dışında (NDC y=13,3) — çip vakasıyla aynı sınıf, parti 25'te bilinçli dokunulmadı | Çip çapası düzeltmesinin desenini `_gridBadge`'e uygulamak (küçük) | bu defter |
