...
| Analiz Merkezi hüküm rozetinin çiğ `{iters}` şablonu basması (parti 32, canlı turda ölçüldü): `verdictBadge` TF ile DOĞRU basıyor, ama `badge()` span'a `data-i18n` yazdığı için hemen ardından gelen `I18N.applyTo` düğümü PARAMETRESİZ `t(key)` ile ezip "CONVERGED ({iters} iterations)" şablonunu koşum kartı + geçmiş şeridinde iki kopya gösteriyordu — parti 28'in {yer_tutucu} kuralının DOM katmanı ikizi; sözlükte 728 parametreli anahtar aynı sınıfın adayı. Kapandı: `translateElement`'e `data-i18n-params` (JSON) desteği (dil değişiminde tf aynı parametrelerle yeniden doldurur) + ezme koruması (params yoksa ve çeviri hâlâ yer tutucu taşıyorsa mevcut metin korunur — sınıfın TAMAMI mekanizma seviyesinde kapalı); `badge()` params'ı öznitelikle taşır. Canlı ölçüm: rozetler "CONVERGED (16592 iterations)", ham yer tutucu SIFIR, TR geçişinde "YAKINSADI (13847 iterasyon)" node-harness'te ölçülü | `test_i18n_param_rozet.py` (4 bekçi: gerçek badge çıktısı + gerçek i18n.js applyTo zinciri, dil değişimi dahil; 2 mutasyon ölçüldü: translateElement eski davranışa dönerse 2 kırmızı, badge params'ı düşürürse 2 kırmızı) |
...
| `MOTOR_TYPES` ÜÇ yerde bağımsız tanımlı (parti 32'de tutarlılık hook'unun AST onarımı ortaya çıkardı, koda karşı teyitli): `hrma/utils/projects.py:56` frozenset, `hrma/validation/experiment_db.py:50` demet, `tests/test_panel_units_v2626.py:69` demet. Değer kümesi bugün aynı ({hybrid, solid, liquid}) ama tek kaynak yok — dördüncü motor tipi geldiği gün biri güncellenir ikisi sessiz çürür | Tek kaynak + iki ithal; sıra/tip semantiği ayrışıyorsa (demetler sıralı geziliyor) gerekçesi yazılır — küçük kalem | bu defter; parti 32 A4 raporu |
| Sıvı sayfasında CFD alanı 3B sahneye İNMİYOR (parti 32 canlı turu, ölçüldü): "3B sahnede göster" → `[contour_mismatch] dr_throat_mm=0,147` — çapa kapısı DÜRÜST reddediyor (hibritte 0,019 mm geçmişti) ama sonuç, sıvı sayfası kendi çözücüsünün alanını kendi sahnesinde hiç gösterememek. Kök aday: ucun "İKİ BOĞAZ YARIÇAPI, BİLEREK" kararı (düğüm yarıçapı vs hücre-sütunu duvar yarıçapı) ile sahne konturunun yeniden-örnekleme toleransının kesişimi — sıvı bell konturunda fark tolerans bandının dışına taşıyor | Hüküm gerekiyor: tolerans ölçülen yeniden-örnekleme adımından mı türetilmeli (kontur yoğunluğuna göre), yoksa sahne konturu uca yayımlanan konturla mı eşitlenmeli; hangisi seçilirse kapının mutasyon bekçisi birlikte güncellenir | bu defter; parti 32 ana model canlı ölçümü |
