...
| ID | İş | Ölçülen durum |
|---|---|---|
| F1 | CFD (lüle iç akışı, ayrılma, şok) | **Gerçek CFD çekirdeği DEPODA** (parti 23+25, `hrma/cfd/`: HLLC+MUSCL axisym Euler, şok yakalama, 36+ bekçi, numba 1,98×); yarı-1B `flow/quasi1d.py` + `flow/separation.py` ayrıca bağlı (101 test). Ürünleşme (dalga B) 16 Ağu'da BİTTİ (parti 26: köprü+uç+Merkez+panel+karakteristik giriş BC). **Viskoz/türbülans FINAL kapsamında (Berke kararı 16 Ağu)**: tasarım belgesi + V5 entegral BL + V0 taşları (parti 26) + **V0-int satır-örtük gevşetme opt-in (parti 27: şoklu vakada 3,9× iterasyon; hüküm-nötrlük 2,2e-12 ölçülü)**. V1 kuyruğu: laminer NS + düşük-Mach ön-koşullama + numba turu |
| F2 | Yanma kararsızlığı | **F2a ÇEKİRDEK DEPODA (16 Ağu, parti 26):** `hrma/stability/` (Γ²/τ_c, chug gerçek çevrimi + nötr eğri, hibrit LFI Karabeyoglu Eq.15 kalibre sabitli + yayımlanmış test tabloları TAM çaprazlı, Culick&Yang lüle sönümü + R_crit, QSHOD zarf mekanizması) — 233 bekçi, hüküm/eşik ayrımı yapısal. Tasarım + 8 Berke kararı: `f2-yanma-tepkisi-tasarimi.md` §8.1. Akustik mod çekirdeği (`acoustic_modes.py`, 36 test) değişmeden ithal ediliyor. **F2b BİTTİ (17 Ağu, parti 27):** hibrit+katı kararlılık blokları yayında (LFI 13 Hz ölçülü, bulk mod kapı-tutarlı, R_crit eşikleri), chug eşiği tek kaynak (kimlik bekçili; 4./5. kopya dar devir bekçisinde), sıvı akustiği merkeze MANİFESTOLU göçtü (1T farkı = tam kök farkı, 3 motorda birebir), künyeli (A,B) bantları + referans-sıcaklıklı termal tablo (birincil kaynak, boşlar beyanlı). Kalan: F2c panel (Analiz Merkezi kiracısı) + hibrit chug bağlaması + form alanları |
