...
class TestWallTemperatureHandoff:
    ...
    def test_without_wall_temps_falls_back_to_chamber_estimate(self, struct):
        ...
        ta = res['thermal_analysis']
        # v2.5.2: fallback iç yüzü servis sınırına DAYAMAK yerine onun
        # WALL_TEMP_SERVICE_FRACTION (0.9) katına kapatıyor. Servis sınırına
        # tam oturtmak her sıcak motoru otomatik CRITICAL yapıyordu; bu
        # katsayı radyasyon-denge tahminine daha yakın. 811 K * 0.9 = 729.9 K.
        from hrma.analysis.structural_analysis import WALL_TEMP_SERVICE_FRACTION
        assert ta['wall_temperature_inner_K'] == pytest.approx(
            811.0 * WALL_TEMP_SERVICE_FRACTION)
...
class TestTwoSafetyFactorReport:
    ...
    def test_hot_uncooled_motor_flagged_by_thermal_margin(self, struct):
        """Sicak sogutmasiz motor SAFE gorunmemeli.

        v2.5.2 MODEL DEGISIKLIGI: eski surum bunu 'termal gerilme baskin'
        diye isaretliyordu, ama o sinyal fiziksel olmayan bir varsayimdan
        (dis cidar = ortam sicakligi, ~430 K gradyan) geliyordu. Ince metal
        cidarda gercek gradyan ~2 K'dir; tehlike gradyan gerilmesi degil,
        cidarin butun olarak servis sinirina yaklasmasidir. Kontrol artik
        sicaklik marjindan yapilir.
        """
        res = struct.analyze_structure(dict(HOT_MOTOR), material='steel_4130')
